...
def test_mincover():
    for size in [10, 20, 30, 40, 50]:
        ...
        assert result_er <= size, f"Test failed for Erdos-Renyi graph size {size}x{size}: Vertex cover size greater than number of nodes."
        # Find the minimum weighted vertex cover
        min_vertex_cover = nx.algorithms.approximation.vertex_cover.min_weighted_vertex_cover(graph_er)
